...
import { IAnyswapCreate2Deployer } from "../typechain-types/contracts/external/interfaces/IAnyswapCreate2Deployer";
...
async function main() {
    ...
    const fsDeployer = ethers.Wallet.fromMnemonic(FUTURESWAP_DEPLOYER_MNEMONIC).connect(ethers.provider);
    ...
    const signedGovernor = await deployAtFixedAddress(new SignedGovernor__factory(fsDeployer), anyswapCreate2Deployer, fsSalt, fsDeployer.address);
    await signedGovernor.connect(fsDeployer).transferOwnership(workDeployer.address);
    const governanceProxy = await deployGovernanceProxy(signedGovernor.address, anyswapCreate2Deployer, workDeployer);
