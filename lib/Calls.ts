...
import { AggregatorV3Interface__factory, DOS__factory, ERC20ChainlinkValueOracle__factory, Governance, GovernanceProxy, GovernanceProxy__factory, Governance__factory, HashNFT, HashNFT__factory, IAssetValueOracle, IERC20 } from "../typechain-types";
...
export async function deployGovernanceProxy(signer: ethers.Signer) {
    return { governanceProxy: await new GovernanceProxy__factory(signer).deploy() };
}
...
export class Chainlink {
    ...
    static async deploy(signer: ethers.Signer, price: number, chainLinkDecimals: number, baseTokenDecimals: number, assetTokenDecimals: number ) {
        const mockChainLink = await waffle.deployMockContract(signer, AggregatorV3Interface__factory.abi);
        await mockChainLink.mock.decimals.returns(chainLinkDecimals);
        const assetOracle = await new ERC20ChainlinkValueOracle__factory(signer).deploy(mockChainLink.address, baseTokenDecimals, assetTokenDecimals);
        const x = new Chainlink(mockChainLink, assetOracle, chainLinkDecimals);
        await x.setPrice(price);
        return x;
    }

    private constructor(mockChainlink: MockContract, assetOracle: IAssetValueOracle, chainlinkDecimals: number) {
...
export async function setupDos(signer: ethers.Signer, governanceProxyAddress: string, ) {
